```/Users/alexian/Documents/IUT/Semestre 5/SAE/lebonmatos/src/components/component/choose.tsx#L1-400
"use client";
...
export interface ComponentItem {
  ...
  color?: string | null;
  type: ComponentType;
  // Additional fields from prisma relations are omitted here for simplicity.
}
...
export type ComponentChooseDrawerProps = {
  ...
  initialOpen?: boolean;
  /**
   * If provided, the drawer will only allow selecting components of this type.
   * This is strongly typed to ComponentType (not a bare string).
   */
 ...
 *
 * - Fetches components from /api/components
 * - Supports searching, pagination and filtering by type
 * - If `allowedType` is provided, the type selector becomes disabled and the list is
 *   constrained to that type. The UI also indicates the enforced type.
 */
export default function ComponentChooseDrawer({
  ...
  // If an allowedType is provided, force the filter to that value
  useEffect(() => {
    if (allowedType) {
  ...
  const filtered = useMemo(() => {
    ...
    return components.filter((c) => {
      // If allowedType is present, enforce it regardless of the type selector
      ...
      >
        <div className="px-4 py-3 border-b flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Parcourez et choisissez un composant depuis la base de données.</p>
            {allowedType ? (
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => setOpen(false)} aria-label="Close" className="p-2 rounded hover:bg-gray-100">
              ✕
            </button>
          </div>
        </div>
          ...
          <div className="mb-4 space-y-3">
            <label className="block">
              ...
              <input
                ...
                placeholder="Rechercher par nom, couleur, prix..."
            ...
            {loading ? (
              ...
              <>
                <ul className="space-y-2">
                  {paginated.map((c) => (
                    <li key={c.id} className="flex items-center justify-between p-3 border rounded-md hover:bg-gray-50">
                      <div>
                      ...
                      <div className="flex items-center gap-3">
                        {typeof c.estimatedPrice === "number" ? <div className="text-sm font-medium">{c.estimatedPrice} €</div> : <div className="text-sm text-gray-400">—</div>}
                        <button type="button" onClick={() => handleSelect(c)} className="px-3 py-1 bg-green-600 text-white rounded hover:bg-green-700">
                          Sélectionner
                        </button>
                      </div>
                    </li>
