...
export type ComponentWithDetails = {
  ...
  Motherboard?: { socket: string; formFactor: string; memorySlots: number; maxMemory: number } | null;
...
export function checkCompatibility(
  ...
  const motherboard = slots.find((s) => s.componentType === "MOTHERBOARD")?.post
    ?.component;
  ...
  const pcCase = slots.find((s) => s.componentType === "CASE")?.post?.component;
...
export function hasStorage(slots: ConfigurationSlot[]): boolean {
  return slots.some(
    (s) => (s.componentType === "SSD" || s.componentType === "HDD") && s.post
  );
}
